...
internal sealed class FakeProvider : IProvider
{
    ...
    public FakeProvider(ProviderCapability capability, Func<ProviderRequest, ProviderResponse>? handler = null)
    {
    ...
    public Task<ProviderResponse> HandleAsync(ProviderRequest request, CancellationToken cancellationToken = default)
    {
        LastRequest = request;
        CallCount++;
