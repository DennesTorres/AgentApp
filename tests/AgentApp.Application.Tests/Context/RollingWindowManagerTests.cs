...
public class RollingWindowManagerTests : IDisposable
{
    ...
    public async Task ArchiveIfNeededAsync_ContentBelowLimit_DoesNotArchive()
    {
        ...
        var archives = await _store.SearchArchivesAsync("Short content.", "session-memory");
    ...
    public async Task ArchiveIfNeededAsync_ContentOverLimit_ArchivesAndClears()
    {
        ...
        var archives = await _store.SearchArchivesAsync("This content", "session-memory");
        Assert.Single(archives);
        var active = await _store.ReadActiveAsync("session-memory");
    ...
    public async Task ArchiveIfNeededAsync_NoRule_DoesNothing()
    {
        ...
        var archives = await _store.SearchArchivesAsync("Some content.", "session-memory");
        Assert.Empty(archives);
        var active = await _store.ReadActiveAsync("session-memory");
        Assert.Equal("Some content.", active);
    }

    [Fact]
    public async Task SearchArchivesAsync_DelegatesToStore()
    {
        await _store.ArchiveAsync("session-memory", "result containing query keyword");
        await _store.ArchiveAsync("session-memory", "another result containing query keyword");
